...
import:  ## Run the import on a database, assumes mba_hierarchy.json and out are in the current dir
	...
	uv run -m app.cli.import_data hierarchy mba_hierarchy.json
	uv run -m app.cli.import_data run ./out --virtual-lab-id $(VIRTUAL_LAB_ID_IMPORT) --project-id $(PROJECT_ID_IMPORT)
