...
def create_one(
    ...
    # Prepare extra_data
    extra_data = {
        "reconstruction_method": validated_body.reconstruction_method.dict()
        if hasattr(validated_body, "reconstruction_method")
        else None,
        "pipeline_state": validated_body.pipeline_state
        if hasattr(validated_body, "pipeline_state")
        else None,
        "is_related_to": validated_body.is_related_to
        if hasattr(validated_body, "is_related_to")
        else [],
    ...
    json_model_data = {
        k: v for k, v in validated_body.dict(exclude_unset=True).items() if k in allowed_fields
    }
    ...
    try:
        if user_id:
            created_by_person = db.execute(
                sa.select(Person).filter_by(id=UUID(user_id))
            ).scalar_one_or_none()
            ...
            json_model_data["updatedBy"] = PersonRead.model_validate(
                created_by_person
            ).model_dump()  # Assume updatedBy is same as createdBy initially
        else:
            raise HTTPException(
                status_code=401, detail="User ID is required for createdBy and updatedBy fields."
            )
    ...
    try:
        if json_model_data.get("species_id"):
            species = db.execute(
                sa.select(Species).filter_by(id=json_model_data["species_id"])
            ).scalar_one_or_none()
            if not species:
                raise HTTPException(
                    status_code=422, detail=f"Invalid species_id: {json_model_data['species_id']}"
                )
        if json_model_data.get("brain_region_id"):
            brain_region_query = sa.select(BrainRegion).filter_by(
                id=json_model_data["brain_region_id"]
            )
            brain_region = db.execute(brain_region_query).scalar_one_or_none()
            if not brain_region:
                raise HTTPException(
                    status_code=422,
                    detail=f"Invalid brain_region_id: {json_model_data['brain_region_id']}",
                )
        if json_model_data.get("license_id"):
            license = db.execute(
                sa.select(License).filter_by(id=json_model_data["license_id"])
            ).scalar_one_or_none()
            if not license:
                raise HTTPException(
                    status_code=422, detail=f"Invalid license_id: {json_model_data['license_id']}"
                )
        if json_model_data.get("contact_id"):
            person = db.execute(
                sa.select(Person).filter_by(id=json_model_data["contact_id"])
            ).scalar_one_or_none()
            if not person:
                raise HTTPException(
                    status_code=422, detail=f"Invalid contact_id: {json_model_data['contact_id']}"
                )
    ...
    if "published_in" in db_model_data and isinstance(db_model_data["published_in"], dict):
        db_model_data["published_in"] = db_model_data["published_in"].get(
    ...
    db_model_data["createdBy_id"] = (
        db_model_data["createdBy"]["id"] if db_model_data.get("createdBy") else None
    )
    db_model_data["updatedBy_id"] = (
        db_model_data["updatedBy"]["id"] if db_model_data.get("updatedBy") else None
    )
    ...
    response_data = {
        ...
        "createdBy": PersonRead.model_validate(row.createdBy).model_dump()
        if row.createdBy
        else None,
        "updatedBy": PersonRead.model_validate(row.updatedBy).model_dump()
        if row.updatedBy
        else None,
    }
    ...
    try:
        response_schema = MORPHOLOGY_TYPE_TO_SCHEMA.get(
            validated_body.morphology_type, CellMorphologyRead
        )
