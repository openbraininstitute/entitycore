...
def build_s3_path(proj_id: UUID, entity_type: EntityType, entity_id: int, filename: str) -> str:
    """Return the path used to store the file on S3."""
    # TODO: decide file organization for efficient partitioning on S3
...
def upload_to_s3(
    ...
    try:
        ...
        L.exception("Error while uploading file to s3://%s/%s", bucket_name, s3_key)
        return False
    L.info("File uploaded successfully to s3://%s/%s", bucket_name, s3_key)
    return True
...
def generate_presigned_url(s3_client: S3Client, bucket_name: str, s3_key: str) -> str | None:
    ...
    try:
        ...
        L.exception("Error generating presigned URL for s3://%s/%s", bucket_name, s3_key)
    return None
