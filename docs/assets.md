...
- Versions/revisions of entities and files are not required (to be agreed).
- Assets are identified by an id/uuid.
- (not specific to assets) Entities must have a unique id that can be used in citations. Examples:
    ...
    - `obi:9d8206c8-f642-4e4c-a2a0-ded3693534b3` (exposes a random uuid, that's globally unique, but not the entity type)
...
- `GET /{entity_type}/{entity_id}/assets` (without version)
- `GET /{entity_type}/{entity_id}/versions/{version_id}/assets` (with version)
...
- `asset`: table of assets
  ...
  - `entity_id`: int  # cannot be ForeignKey, but it allows many-to-one relation, since the same asset cannot be shared across multiple entities
  - `entity_type`: EntityType  # needed to look up the correct entity table
