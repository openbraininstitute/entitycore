...
def test_create_strain(client):
    ...
    assert "id" in data
    response = client.post("/strain/", json={"name": "Test Strain", "taxonomy_id": "Taxonomy ID", "species_id": data["id"]})
    assert response.status_code == 200
    data = response.json()
...
def test_create_reconstruction_morphology(client):
    ...
    species_id = data["id"]
    response = client.post("/strain/", json={"name": "Test Strain", "taxonomy_id": "Taxonomy ID", "species_id": species_id})
    ...
    response = client.post("/brain_region/", json={"name": "Test Brain Region", "ontology_id": ontology_id})
    ...
    response = client.post("/reconstruction_morphology/", json= {
    ...
    assert response.status_code == 200, f'Failed to create reconstruction morphology: {response.text}'
    data = response.json()
    print(data )
    assert data["brain_region"]["id"] == brain_region_id, f'Failed to get id for reconstruction morphology: {data}'
    assert data["species"]["id"] == species_id, f'Failed to get species_id for reconstruction morphology: {data}'
    assert data["strain"]["id"] == strain_id, f'Failed to get strain_id for reconstruction morphology: {data}'
    assert data["description"] == morph_description, f'Failed to get description for reconstruction morphology: {data}'
    assert data["name"] == morph_name, f'Failed to get name for reconstruction morphology: {data}'

# def test_create_annotation(client)
#     response = client.post("/morphology_feature_annotation/", json={"name": "Test Annotation",
#                                                                      "description": "Test Description"})

#     assert response.status_code == 200
#     data = response.json()
#     assert data["name"] == "Test Annotation"
#     assert "id" in data
