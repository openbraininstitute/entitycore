...
def test_brain_region_id(db, client, client_admin, person_id, species_id):
    ...
    # test search by hierarchy_id
    response = client.get(ROUTE, params={"hierarchy_id": str(hierarchy_name.id)})
    assert len(response.json()["data"]) == 4

    response = client.get(ROUTE, params={"hierarchy_id": "00000000-7000-4000-0000-000000000000"})
    assert len(response.json()["data"]) == 0
