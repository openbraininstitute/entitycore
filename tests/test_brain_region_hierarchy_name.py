...
ROUTE = "/brain-region-hierarchy-name"


def test_brain_region_hierarchy_name(client, brain_region_hierarchy_name_id):
    response = client.get(f"{ROUTE}/{brain_region_hierarchy_name_id}")
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == "AIBS"
    assert data["id"] == str(brain_region_hierarchy_name_id)
    ...
    assert data[0]["id"] == str(brain_region_hierarchy_name_id)
...
def test_hierarchy_tree(db, client, brain_region_hierarchy_name_id):
    test_brain_region.add_brain_region_hierarchy(
        db, test_brain_region.HIERARCHY, brain_region_hierarchy_name_id
    )

    response = client.get(f"{ROUTE}/{brain_region_hierarchy_name_id}/hierarchy")
    assert response.status_code == 200
    data = response.json()
