...
from app.db.model import Campaign, TaskConfig
...
ROUTE = "campaign"
ADMIN_ROUTE = "/admin/campaign"


@pytest.fixture
def json_data(campaign_json_data):
...
def test_filtering_ordering(client, models):
    ...
    data = _req({"task_config__name": "config-2"})
    ...
    data = _req({"task_config__name": "config-2", "order_by": "name"})
    assert [d["name"] for d in data] == ["campaign-1", "campaign-2"]
