...
from app.db.types import ActivityType, EntityType, ExecutorType
...
@pytest.fixture
def json_data(circuit_extraction_config_id, root_circuit):
    return {
...
def _assert_read_response(data, json_data, *, empty_ids=False):
    ...
    assert data["type"] == ActivityType.circuit_extraction_execution
...
@pytest.fixture
def models(morphology_id, circuit_extraction_config_id, ion_channel_model_id, create_id):
    return [
        create_id(
            used_ids=[circuit_extraction_config_id],
            generated_ids=[],
        ),
        create_id(
            used_ids=[circuit_extraction_config_id],
        ...
        create_id(
            used_ids=[circuit_extraction_config_id, ion_channel_model_id],
            generated_ids=[
        ...
        create_id(
            used_ids=[str(morphology_id), circuit_extraction_config_id],
            generated_ids=[ion_channel_model_id],
        ),
    ]


def test_filtering(client, models, circuit_extraction_config_id, ion_channel_model_id):
    data = assert_request(client.get, url=ROUTE).json()["data"]
    assert len(data) == len(models)

    data = assert_request(
        client.get, url=ROUTE, params={"used__id": circuit_extraction_config_id}
    ).json()["data"]
    assert len(data) == 5

    data = assert_request(
        client.get, url=ROUTE, params={"generated__id": circuit_extraction_config_id}
    ).json()["data"]
    assert len(data) == 2
    ...
    data = assert_request(
        ...
        url=ROUTE,
        params={
            "used__id": circuit_extraction_config_id,
            "generated__id": circuit_extraction_config_id,
        },
    ).json()["data"]
    assert len(data) == 2
