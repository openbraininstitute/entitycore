...
def _assert_read_response(data, json_data, *, empty_ids=False):
    ...
    if empty_ids:
        ...
        check_nested_entity_read_response(
            ...
            expected_entity_type=str(EntityType.task_config),
...
@pytest.fixture
def models(create_id, task_config_id, morphology_id_1, morphology_id_2):
    return [
        create_id(
            used_ids=[task_config_id],
            generated_ids=[],
        ),
        create_id(
            used_ids=[task_config_id],
...
def test_filtering(client, models, task_config_id, morphology_id_1, morphology_id_2):
    ...
    data = assert_request(
        client.get,
        url=ROUTE,
        params={"used__id": task_config_id},
    ).json()["data"]
    assert len(data) == 3
    ...
    data = assert_request(
        client.get,
        url=ROUTE,
        params={"used__id__in": [task_config_id]},
    ).json()["data"]
    assert len(data) == 3
