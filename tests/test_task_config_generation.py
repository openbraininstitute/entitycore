...
@pytest.fixture
def json_data(campaign_id, task_config_id):
    return {
        "start_time": str(datetime.now(UTC)),
        "end_time": str(datetime.now(UTC)),
        "used_ids": [campaign_id],
        "generated_ids": [task_config_id],
    }
...
def _assert_read_response(data, json_data, *, empty_ids=False):
    ...
    assert data["type"] == ActivityType.task_config_generation.value
...
@pytest.fixture
def task_config_id_1(client, campaign_id):
...
@pytest.fixture
def task_config_id_2(client, campaign_id):
    return create_task_config_id(
        client=client,
        campaign_id=campaign_id,
    )
...
def models(
    ...
    return [
        ...
        create_id(
            used_ids=[campaign_id],
            generated_ids=[task_config_id_1],
        ),
        create_id(
            used_ids=[campaign_id],
            generated_ids=[
...
def test_filtering(
    ...
    data = assert_request(
        ...
        url=ROUTE,
        params={
            "used__id": campaign_id,
            "generated__id": task_config_id_2,
        },
    ).json()["data"]
    assert len(data) == 1
    ...
    data = assert_request(
        client.get,
        url=ROUTE,
        params={"used__id__in": [campaign_id]},
    ).json()["data"]
    assert len(data) == 3
    ...
    data = assert_request(
        client.get,
        url=ROUTE,
        params={"generated__id__in": [task_config_id_2]},
    ).json()["data"]
    assert len(data) == 1
...
def test_update_one__fail_if_generated_ids_exists(
    ...
    check_activity_update_one__fail_if_generated_ids_exists(
        client=client,
        route=ROUTE,
        entity_id_1=campaign_id,
        entity_id_2=task_config_id,
        constructor_func=create_id,
    )
